...
const DICT: Record<string, [string, string]> = {
  ...
  'search.unsupported': [
    '跨表搜索暂仅支持 MySQL / PostgreSQL 系方言',
    'Cross-table search supports MySQL / PostgreSQL dialects only',
  ],
  'search.idle': [
    '输入一个值，扫描该连接所有字符列（VARCHAR/TEXT/JSON…）',
    'Enter a value to scan every text column (VARCHAR/TEXT/JSON…) on this connection',
  ],
  'search.searching': ['搜索中… {done}/{total} 张表', 'Searching… {done}/{total} tables'],
  'search.summary': ['命中 {tables} 张表 · 共 {rows} 行', '{tables} table(s) · {rows} row(s) matched'],
  'search.matchedCols': ['匹配列', 'Matched columns'],
  'search.previewNote': ['仅显示前 {n} / 共 {total} 行', 'Showing first {n} of {total} rows'],
  'search.valueLabel': ['搜索值', 'Value'],
  'search.tableFilterLabel': ['表名过滤', 'Table filter'],
  ...
  'conn.importFile': ['导入文件', 'Import file'],
  'conn.ssh.enable': ['启用 SSH 隧道', 'Enable SSH tunnel'],
  'conn.ssh.test': ['测试 SSH', 'Test SSH'],
  ...
  'import.paste': ['粘贴剪贴板', 'Paste from clipboard'],
  'import.fromClipboard': ['（剪贴板）', '(clipboard)'],
  'import.clipboardFail': ['读取剪贴板失败（需授权）', 'Could not read clipboard (permission?)'],
  'import.clipboardEmpty': ['剪贴板是空的', 'Clipboard is empty'],
