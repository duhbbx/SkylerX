...
## 11. 移行評価(MigrationAssessWizard)

Oracle / DM のソース DB を、openGauss 系の国産 DB(Vastbase / openGauss)または DM への移行可否レポートに変換します。脱 Oracle(「去O」)案件のプリセールス / DBA 向け:移行工数を見積もる*前*に、ソースに何がどれだけあり、どれだけ大きく、どれだけ難しいかを把握します。

**入口**:コマンドパレット `act:mig-assess`(「移行評価」で検索)、または Oracle / DM 接続を右クリック → `🧭 移行評価…`(その接続をソースに自動入力)。コードは `packages/ui/src/components/MigrationAssessWizard.vue`、評価ロジックはすべて `packages/ui/src/migrate/` にあります。

### 5 ステップウィザード

| ステップ | 内容 |
|---|---|
| 1 接続 | ソース(プロファイル可能な方言)+ ターゲット(変換経路のある方言)を選択 |
| 2 プロファイリング | データベース / スキーマ一覧(システムを除外)、移行対象にチェック、全オブジェクト棚卸し + リスク指標 |
| 3 評価 | 選択スキーマのオブジェクトを取得し、各々を A/B/C/D で格付け + 全体「準備度」スコア |
| 4 AI 変換 | C 級(PL/SQL 本体 / 複雑な SQL)を AI に渡してターゲット方言へ翻訳(読み取り専用) |
| 5 レポート | 集計し Excel / Word / PDF / Markdown へエクスポート |

### hub-and-spoke 変換アーキテクチャ

ソース→ターゲットのペアごとの変換器(N×M で爆発)ではなく、中間に抽象データベースモデル(Logical IR)を置きます:

```
ソース ──parse──▶ Logical IR ──emit──▶ ターゲット
```

各方言は parser か emitter を 1 つ用意するだけで済むため、DB 追加は N×M ではなく N+M です。コード:`migrate/ir.ts`(モデル)、`migrate/convert.ts`(編成)、`migrate/dialects/{oracle,postgres,dameng}.ts`。**境界**:構造オブジェクト(テーブル / カラム / 型 / 制約)は決定論的 IR を通り、手続き型オブジェクト(プロシージャ / 関数 / パッケージ / トリガー / ビュー)は元の DDL を保持して AI(`migrate/aiConvert.ts`)が翻訳します。正規表現では意味的な書き換えができないためです。

### ソースのプロファイリング

`migrate/profile.ts` + `migrate/profilers/{oracle,postgres}.ts`、ソースファミリーごとにカタログクエリ一式。17 種類のオブジェクトを棚卸しし、**未対応のオブジェクトは偽の 0 ではなく `—`(null)で表示**します:

> テーブル · ビュー · マテリアライズドビュー · パーティションテーブル · インデックス · 主キー · 外部キー · ユニーク制約 · チェック制約 · シーケンス · 関数 · プロシージャ · パッケージ · トリガー · 型 · シノニム · DB リンク

さらに移行リスク指標:**主キーなしテーブル数**(CDC の最大の落とし穴)、**LOB 列数**(データ移行時間の大半)、**トリガー付きテーブル数**、**コメント付きテーブル数**;加えて行数バケット(≥100万 / ≥1000万 / ≥1億)、テーブルスペースサイズ、最大テーブル。行数はカタログ統計の推定値(`reltuples` / `num_rows`)を使い、正確な `COUNT(*)` はしないため、数十億行のテーブルでも数秒で返ります。`dba_segments` が読めない(DBA 権限なし)場合は自動的に縮退します(サイズ 0 + 通知)。

### ドキュメント出力

レポート画面に 4 つのボタン。すべて既存依存(`xlsx` / `marked`)を再利用し、新規ライブラリなし:

| 形式 | 方法 |
|---|---|
| Excel `.xlsx` | 複数シート:概要 / 棚卸し / 大テーブル / 評価明細 / AI 変換 |
| Word `.doc` | Markdown をスタイル付き HTML にレンダリングし、Word でネイティブに開く |
| PDF | 同じ HTML を新ウィンドウで自動印刷 → Chromium の「PDF として保存」 |
| Markdown `.md` | プレーンテキストレポート |

> 格付け:**A 自動**(決定論的、そのまま使用)· **B 補助**(型 / 意味の差、要抜き取り確認)· **C 手動**(PL/SQL 本体や独自構文、AI + 人手が必要)· **D ブロック**(空間 / 外部パッケージなど等価物なし、アーキテクチャレベルの検討が必要)。準備度 = オブジェクト格付けの加重平均(A=100 / B=80 / C=40 / D=0)。

### v0.5.8:評価から実行までの完全ループ

ウィザードは 5 ステップから 6 ステップに拡張(**データ移行**ステップを追加)、
エンジンは実際に移行を「やり切る」のに必要な機能を備えました:

- **変換の忠実度** — 型に加えて、主キー/外部キー/ユニーク/チェック制約、
  インデックス、シーケンス、コメントを依存順(シーケンス → テーブル → コメント
  → インデックス → 外部キー)で、そのまま実行できる一括スクリプトとして出力。
- **完全スクリプト生成 + ワンクリック構築** — ソース構造を読む → `convertSchema`
  → ターゲットで実行。PG 系はアトミック(トランザクション内で dry-run してから
  commit、いずれか失敗で全ロールバック)、非トランザクション系は逐次実行・初回
  エラーで停止。
- **AI 自己修復ループ** — AI が変換した DDL をターゲットのトランザクション内で
  試走(`BEGIN…ROLLBACK`、痕跡なし)、エラーを AI に返して自動修正、最大 3 回。
  通れば ✅。
- **データ移行** — テーブル単位のチャンク搬送(キャンセル可 + テーブル単位の
  再開)+ 行数照合 + 主キー列レベル照合(非 NULL 数 / 最小 / 最大)、テーブル
  ごとに ✅/❌。
- **ソース方言の拡張** — Oracle/DM に加え、**PG 系 / MySQL / SQL Server** をソース
  に(脱 PG / 脱 MySQL / 脱 SQL Server)。ターゲットに **MySQL 系**(OceanBase /
  TiDB / GBase など)を追加。DB 追加は引き続き N+M。

> 主要パス(変換 / プロファイリング / 検証 / 構築 / データ搬送 / スキーマ全体の
> round-trip)は実稼働の openGauss カーネル DB(Vastbase)で検証済み。
