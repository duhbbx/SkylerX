...
| ✅ | 查询历史 + 标签 + 置顶 | 已上线 |
| ✅ | **Notebook 模式** | 多 cell 混排 SQL / Markdown,本地持久化;类似 Jupyter |
...
| ✅ | **Linter 自定义规则编辑器** | 用户自定义禁用模式 / 风格规则(正则匹配 + 严重级别) |
...
| ✅ | **查询结果对比(diff)** | 两次结果集按行/单元格 diff,标新增 / 删除 / 改动 |
| ✅ | **导出脱敏** | 开启脱敏后,复制 / 导出(CSV/JSON/SQL/…)整列按规则打码,跟网格显示一致(不再「看着打码、导出明文」) |
...
| ✅ | **ER 图自动布局** | 从实时 schema 反向出图,外键自动连线(子→父),按列数缩放、主键表高亮、聚焦某表+邻居,可导出 PNG / SVG |
| 🔵 | **正向工程** | 在 ER 图改完 → 生成 migration SQL |
| ✅ | **跨库迁移 v2** | hub-and-spoke IR:MySQL/Oracle/DM/SQLServer 解析 → PG/Oracle/DM/MySQL 输出,带类型 / 索引 / 视图 / 外键全套;数据搬迁(分块参数化 + 增量 + 校验) |
| ⚪ | **dbt 集成** | 识别 dbt 项目,模型预览 / lineage |
| ✅ | **数据 lineage 图** | 解析 SQL 出表级血缘(列级在路线图) |
...
| ✅ | **存储增长趋势预测** | 采集库/表体积快照,按历史拟合 7/30/90 天容量曲线 + 到顶预警 |
...
| ✅ | **RAG over schema + docs** | schema(表/视图/函数) + 文档切 chunk → 向量(OpenAI 兼容 /v1/embeddings)+ BM25 混合检索(RRF 融合)+ 相关度地板,只把相关表注入 AI 上下文;无 embedding 时优雅退化词法 |
...
| ✅ | **各方言对象类型补全** | Oracle/DM(含「类型」在 DM 的 `CLASS` 修正)、Vastbase/openGauss 与整个 PG 家族(物化视图 / 存储过程 / 类型,openGauss 另加包 / 同义词)、SQL Server(函数 / 存储过程 / 触发器 / 序列 / 类型 / 同义词)都补齐 |
| ✅ | **一键排除系统库/Schema** | 可见库/Schema 配置里一键取消勾选系统库/schema(mysql / pg_catalog / SYS / SYSAUDITOR …),用户对象不动;单层方言(MySQL 等)不再显示无意义的 schema 下拉 |
| ✅ | **复制连接内容** | 右键连接 →「复制连接内容」二级菜单:JDBC URL / JSON / 多行 / 单行(分号),一律不含密码 |
| ✅ | **移动到分组(combobox)** | 批量移动到分组:下拉选已有分组或手输新名(前后 trim,不存在则建);留空 = 取消分组 |
...
| 时间 | 重大节点 |
|---|---|
| 2026-06 | RAG over schema(向量+BM25 混合检索)· ER 图 + PNG/SVG 导出 · 查询结果对比 · 导出脱敏 · 各方言导航对象类型补全 · 跨方言 reader 真库验证(DM/Oracle/MySQL/Vastbase) |
