...
### v0.5.8: 평가에서 실행까지의 완전한 루프

마법사가 5단계에서 6단계로 확장(**데이터 마이그레이션** 단계 추가)되고, 엔진은
실제로 마이그레이션을 끝까지 수행하는 데 필요한 기능을 갖췄습니다:

- **변환 충실도** — 타입 외에 기본키/외래키/유니크/체크 제약, 인덱스, 시퀀스,
  주석을 의존성 순서(시퀀스 → 테이블 → 주석 → 인덱스 → 외래키)로 바로 실행
  가능한 일괄 스크립트로 생성.
- **전체 스크립트 생성 + 원클릭 구축** — 소스 구조 읽기 → `convertSchema` →
  타겟에서 실행. PG 계열은 원자적(트랜잭션 내 dry-run 후 commit, 하나라도 실패하면
  전체 롤백), 비트랜잭션 계열은 순차 실행 후 첫 오류에서 중단.
- **AI 자가 수리 루프** — AI가 변환한 DDL을 타겟 트랜잭션 내에서 시험 실행
  (`BEGIN…ROLLBACK`, 흔적 없음), 오류를 AI에 다시 보내 자동 수정, 최대 3회.
  통과하면 ✅.
- **데이터 마이그레이션** — 테이블별 청크 복사(취소 가능 + 테이블 단위 재개) +
  행 수 대조 + 기본키 열 수준 대조(비널 수 / 최소 / 최대), 테이블별 ✅/❌.
- **소스 방언 확장** — Oracle/DM 외에 **PG 계열 / MySQL / SQL Server**를 소스로
  (탈 PG / 탈 MySQL / 탈 SQL Server). 타겟에 **MySQL 계열**(OceanBase / TiDB /
  GBase 등) 추가. DB 추가는 여전히 N+M.

> 핵심 경로(변환 / 프로파일링 / 검증 / 구축 / 데이터 복사 / 스키마 전체
> round-trip)는 실제 openGauss 커널 DB(Vastbase)에서 검증했습니다.
