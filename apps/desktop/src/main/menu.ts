  ...
  const template: MenuItemConstructorOptions[] = [
    ...
    // ── 工具（DBA 重活儿 + AI 入口）──
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        { label: '结构对比 (Schema diff)…', click: send('schema-diff') },
        { label: '数据对比 (Data diff)…', click: send('data-diff') },
        { label: 'Schema 快照…', click: send('snapshots') },
        { label: 'ER 关系图…', click: send('er-diagram') },
        { label: '信创迁移评估…', click: send('mig-assess') },
        { type: 'separator' },
        ...
        { label: 'AI 助手对话框…', click: send('ai-assistant') },
        { label: 'AI 知识库 (RAG)…', click: send('rag') },
        { label: '跨方言 SQL 翻译…', click: send('translate') },
      ],
    },
